...
// ✅ STEP 1: Replace this with your actual Formspree form ID
// Go to formspree.io → New Form → copy the ID from the endpoint URL
const FORMSPREE_FORM_ID = "xnjrzbab"; // e.g. "xpwzabcd"
...
async function sendEmailNotification(data: {
  ...
  try {
    await fetch(`https://formspree.io/f/${FORMSPREE_FORM_ID}`, {
      ...
      body: JSON.stringify({
        _subject: `🔔 New Quote Request — ${data.brand} ${data.model} (${data.service})`,
        Name: `${data.firstName} ${data.lastName}`,
        Phone: data.phone,
        Email: data.email,
        Device: `${data.brand} ${data.grouping} — ${data.model}`,
    ...
    // Silently fail — don't block the user experience if email fails
...
function GetAQuote(): JSX.Element {
  ...
  const [submittedServiceData, setSubmittedServiceData] =
          ...
          <div style={{ display: "flex", justifyContent: "center" }}>
            <Submit
              ...
              onClick={async () => {
                ...
                // ✅ NEW: Send email notification to you
                ...
                const matchedService =
                  allServices.find(
                    (s) => s.name.toLowerCase() === serviceLower,
                  ) ??
                  allServices.find((s) =>
          ...
          <div className="quote-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="info-form-modal-card">
              ...
              <div className="info-form-modal-card-body">
                ...
                <p className="info-form-modal-service">
